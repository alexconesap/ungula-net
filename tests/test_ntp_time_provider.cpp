...
namespace {
    ...
    TEST_F(NtpTimeProviderTest, InvalidBeforeNtpSync) {
        ...
        EXPECT_EQ(p.nowMs(), 0U);
    }

    TEST_F(NtpTimeProviderTest, ValidOnceNtpReportsSynced) {
        g_fake.synced = true;
        ...
        // Low 32 bits of (1'700'000'000 * 1000) = 1'700'000'000'000.
        const uint64_t fullMs = static_cast<uint64_t>(g_fake.epoch) * 1000ULL;
        const uint32_t expected = static_cast<uint32_t>(fullMs);
    ...
    TEST_F(NtpTimeProviderTest, NowAdvancesWithLocalTickBetweenRefreshes) {
        ...
        const uint32_t t1 = p.nowMs();
        EXPECT_EQ(t1 - t0, 500U);

        g_fake.localTick = 2'250;
        const uint32_t t2 = p.nowMs();
        EXPECT_EQ(t2 - t0, 1'250U);
    }
    ...
    TEST_F(NtpTimeProviderTest, TimeControlFallsBackToLocalWhenProviderInvalid) {
        ...
        const uint32_t t = TimeControl::now();
