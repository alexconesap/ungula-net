...
namespace ungula::net::wifi
{
    ...
    bool ap_init(const WifiApConfig &config)
    {
        ...
        if (config.password != nullptr && config.password[0] != '\0') {
            std::strncpy(reinterpret_cast<char *>(ap_cfg.ap.password), config.password, sizeof(ap_cfg.ap.password) - 1);
        ...
        if (s_ap_netif != nullptr) {
            ...
            esp_netif_dhcps_option(s_ap_netif, ESP_NETIF_OP_SET, ESP_NETIF_DOMAIN_NAME_SERVER, &dhcps_offer_dns,
    ...
    const char *ap_get_mac()
    {
        ...
        if (esp_wifi_get_mac(WIFI_IF_AP, mac) == ESP_OK) {
            snprintf(s_ap_mac_str, sizeof(s_ap_mac_str), "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2],
                     mac[3], mac[4], mac[5]);
        }
        return s_ap_mac_str;
    }
